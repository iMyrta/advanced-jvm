...
class DataConverterSourceBuilder {
    ...
    DataConverterSourceBuilder(DataClass dataClass) {
    ...
    FileObject buildSchema(Filer filer) throws Exception {
        ...
        try (PrintWriter out = new PrintWriter(file.openWriter())) {
            ...
            for (int i = 0; i < dataClass.getFields().size(); i++) {
                DataField field = dataClass.getFields().get(i);
                String protobufType;
    ...
    void buildConverterClass(Filer filer) throws Exception {
        ...
        TypeSpec type = TypeSpec.classBuilder(converterClassName)
                .addAnnotation(ProtobufDataConverter.class)
                .addModifiers(Modifier.PUBLIC)
                .addSuperinterface(ParameterizedTypeName.get(ClassName.get(DataConverter.class), TypeVariableName.get(dataClassName)))
    ...
    private MethodSpec fromBytes(String dataClassName, String innerClass) {
        ...
        for (DataField field : dataClass.getFields()) {
